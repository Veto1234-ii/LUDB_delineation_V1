...
def save_model_3(binary_dataset, POINT_TYPE, LEAD_NAME, epochs, binary_dataset_seg, input_size):
    ...
    # signals_train_neg, labels_train_neg = binary_dataset_negative.get_train()
    # class3 = torch.from_numpy(signals_train_neg[labels_train_neg == 1]).float()[:,230:]

    train_loader = DataLoader(class1, batch_size=50, shuffle=False)
    train_loader_2 = DataLoader(class2, batch_size=50, shuffle=False)

    model = CNN(32, 128, 7, 3, 0.339, input_size)
    ...
    torch.save(model, f"{SAVED_NETS_PATH}/{binary_dataset.get_name()}_negative30+50_{timestamp}.pth")
