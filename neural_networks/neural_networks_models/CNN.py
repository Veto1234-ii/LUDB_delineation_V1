...
class CNN(nn.Module):
    def __init__(self, conv1_out=32, conv2_out=64, kernel1_size=3, kernel2_size=2, dropout_rate=0.2, input_size=400):
        ...
        # Динамический расчёт размера (аналогично первой версии)
        with torch.no_grad():
...
def save_model(binary_dataset, POINT_TYPE, LEAD_NAME, epochs):
    ...
    model = CNN(32, 128, 7, 3, 0.339, 300)
    ...
    path_to_dataset = Path('D:\\PycharmProjects\\LUDB_delineation_V1\\ecg_data_200.json')
    ...
    torch.save(model, f"SAVED_NETS/{binary_dataset.get_name()}_{epochs}_{timestamp}.pth")
